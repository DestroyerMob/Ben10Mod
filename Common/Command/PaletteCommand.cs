...
public class PaletteCommand : ModCommand {
    ...
    public override string Description => "Opens the transformation palette customization menu for the current or selected form.";
    public override CommandType Type => CommandType.Chat;

    public override void Action(CommandCaller caller, string input, string[] args) {
        if (Main.dedServ || caller.Player.whoAmI != Main.myPlayer) {
            Main.NewText("Palette customization can only be opened from a local client.", Color.Orange);
            return;
        }

        UISystem uiSystem = ModContent.GetInstance<UISystem>();
        if (uiSystem?.MyInterface == null || uiSystem.TPS == null) {
            Main.NewText("The palette menu is not available right now.", Color.Red);
            return;
        }
