...
You start with `Heatblast` unlocked.

Your first goals are:

1. Craft any `Plumber's Badge`.
2. Get a `Prototype Omnitrix`.
3. Equip the Omnitrix in the custom Omnitrix slot.
...
- requires at least `55%` pickaxe power
- smelts into `Congealed Codon Bars`
- is used to craft the `Prototype Omnitrix`
...
- classic timeout-style transformations
- evolves into the `Recalibrated Omnitrix`
...
- defeating `Wall of Flesh` while using the `Prototype Omnitrix`
- or crafting it from `Prototype Omnitrix`, `8 Soul of Night`, and `8 Soul of Light`
...
Pre-hardmode unlocks:

- `Diamondhead`: `King Slime`
- `XLR8`: `Eye of Cthulhu`
- `Four Arms`: `Brain of Cthulhu` or `Eater of Worlds`
- `Stinkfly`: `Queen Bee`
- `Buzzshock`: `Skeletron`
- `Wildvine`: `Deerclops`
- `Rath`: `Wall of Flesh`
...
- `Chromastone`: `Queen Slime`
- `Humungousaur`: `The Destroyer`
- `Eye Guy`: `The Twins`
- `Echo Echo`: `Skeletron Prime`
- `Swampfire`: `Plantera`
- `Armodrillo`: `Golem`
- `Jetray`: `Duke Fishron`
...
- make sure an Omnitrix is equipped
- make sure the alien is assigned to one of your five roster slots
- make sure you are not on cooldown
